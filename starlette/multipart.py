...
from starlette.parse_headers import parse_options_header
import io
...
class MultiPartParser:
    def __init__(
        ...
        self.cls = dict if cls is None else cls
    ...
    class LineParser(object):
        ...
        def _state_headers(self, line):
            ...
            if not line:
                self._headers = MutableHeaders(self._raw_headers)
                return self._start_content()
            if line[0] in b" \t" and self._raw_headers:
                key, value = self._raw_headers[-1]
                self._raw_headers[-1] = (key, value + b"\n " + line[1:])
            else:
            ...
            if ev == "begin_file":
                ...
                if self._is_file:
                    ...
                    return (
                        ...
                        (
                            self._name,
                            FileStorage(self._container, self._filename)
                            # FileStorage(
                            #     self._container,
                            #     self._filename,
                            #     self._name,
    ...
    async def parse(self, stream, boundary, content_length):
        ret = {}
