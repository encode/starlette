...
async def app(scope, receive, send):
    ...
    if data.headers is not None:
        output["__headers"] = [
            (field_name, dict(headers.items()))
            for field_name, headers in data.headers.multi_items()
        ]
...
def test_multipart_request_data(tmpdir, test_client_factory):
    ...
    assert response.json() == {
        "some": "data",
        "__headers": [["some", {"content-disposition": 'form-data; name="some"'}]],
    }
...
def test_multi_items(tmpdir, test_client_factory):
    ...
    with open(path1, "rb") as f1, open(path2, "rb") as f2:
        ...
        assert response.json() == {
            ...
            "__headers": [
                ["test1", {"content-disposition": 'form-data; name="test1"'}],
                [
                    "test1",
                    {
                        "content-disposition": 'form-data; name="test1"; filename="test1.txt"'  # noqa: E501
...
def test_multipart_request_mixed_files_and_data(tmpdir, test_client_factory):
    ...
    assert response.json() == {
        ...
        "field1": "value1",
        "__headers": [
            ["field0", {"content-disposition": 'form-data; name="field0"'}],
            [
                "file",
                {
            ...
            ["field1", {"content-disposition": 'form-data; name="field1"'}],
        ],
    }
...
def test_multipart_request_with_encoded_value(tmpdir, test_client_factory):
    ...
    assert response.json() == {
        "value": "Transférer",
        "__headers": [["value", {"content-disposition": 'form-data; name="value"'}]],
    }
...
def test_multipart_multi_field_app_reads_body(tmpdir, test_client_factory):
    ...
    assert response.json() == {
        ...
        "second": "key pair",
        "__headers": [
            ["some", {"content-disposition": 'form-data; name="some"'}],
            ["second", {"content-disposition": 'form-data; name="second"'}],
        ],
    }
